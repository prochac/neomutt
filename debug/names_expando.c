...
const char *name_expando_uid_pgp_key_gpgme(int uid)
{
  switch (uid)
  {
    DEBUG_NAME(ED_GPG_DATE);
    DEBUG_NAME(ED_GPG_KEY_ALGORITHM);
    DEBUG_NAME(ED_GPG_KEY_CAPABILITIES);
    DEBUG_NAME(ED_GPG_KEY_FINGERPRINT);
    DEBUG_NAME(ED_GPG_KEY_FLAGS);
    DEBUG_NAME(ED_GPG_KEY_ID);
    DEBUG_NAME(ED_GPG_KEY_LENGTH);
    DEBUG_NAME(ED_GPG_NUMBER);
    DEBUG_NAME(ED_GPG_PKEY_ALGORITHM);
    DEBUG_NAME(ED_GPG_PKEY_CAPABILITIES);
    DEBUG_NAME(ED_GPG_PKEY_FINGERPRINT);
    DEBUG_NAME(ED_GPG_PKEY_FLAGS);
    DEBUG_NAME(ED_GPG_PKEY_ID);
    DEBUG_NAME(ED_GPG_PKEY_LENGTH);
    DEBUG_NAME(ED_GPG_PROTOCOL);
    DEBUG_NAME(ED_GPG_TRUST);
    DEBUG_NAME(ED_GPG_USER_ID);
    DEBUG_DEFAULT;
  }
}
#endif
...
const char *name_expando_uid(enum ExpandoDomain did, int uid)
{
  ...
  {
    case ED_ALL:
      return name_expando_uid_all(uid);
    case ED_ALIAS:
...
#ifdef CRYPT_BACKEND_GPGME
    case ED_GPGME_KEY:
      return name_expando_uid_pgp_key_gpgme(uid);
#endif
    case ED_HISTORY:
