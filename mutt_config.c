...
/**
 * AttachFormatData - Expando definitions
 *
 * Config:
 * - $attach_format
 *
...
static const struct ExpandoDefinition AttachFormatData[] = {
  ...
  { "d", "description",      ED_BODY,   ED_BOD_DESCRIPTION,      E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "e", "mime-encoding",    ED_BODY,   ED_BOD_MIME_ENCODING,    E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "f", "file",             ED_BODY,   ED_BOD_FILE,             E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "F", "file-disposition", ED_BODY,   ED_BOD_FILE_DISPOSITION, E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "I", "disposition",      ED_BODY,   ED_BOD_DISPOSITION,      E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "m", "mime-major",       ED_BODY,   ED_BOD_MIME_MAJOR,       E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "M", "mime-minor",       ED_BODY,   ED_BOD_MIME_MINOR,       E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "n", "number",           ED_ATTACH, ED_ATT_NUMBER,           E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "Q", "attach-qualifies", ED_BODY,   ED_BOD_ATTACH_QUALIFIES, E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "s", "file-size",        ED_BODY,   ED_BOD_FILE_SIZE,        E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "t", "tagged",           ED_BODY,   ED_BOD_TAGGED,           E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "T", "tree",             ED_ATTACH, ED_ATT_TREE,             E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "u", "unlink",           ED_BODY,   ED_BOD_UNLINK,           E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "X", "attach-count",     ED_BODY,   ED_BOD_ATTACH_COUNT,     E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { NULL, NULL, 0, -1, -1, 0, NULL }
  // clang-format on
};

/**
 * parse_index_date_recv_local - XXX - Implements ::expando_parser_t - @ingroup expando_parser_api
 */
struct ExpandoNode *parse_index_date_recv_local(const char *s, const char **parsed_until,
                                                int did, int uid,
...
/**
 * parse_index_date_local - XXX - Implements ::expando_parser_t - @ingroup expando_parser_api
 */
struct ExpandoNode *parse_index_date_local(const char *s, const char **parsed_until,
...
/**
 * parse_index_date - XXX - Implements ::expando_parser_t - @ingroup expando_parser_api
 */
struct ExpandoNode *parse_index_date(const char *s, const char **parsed_until,
                                     int did, int uid, struct ExpandoParseError *error)
{
  // s-1 is always something valid
  if (*(s - 1) == '<')
...
/**
 * parse_index_hook - XXX - Implements ::expando_parser_t - @ingroup expando_parser_api
 */
struct ExpandoNode *parse_index_hook(const char *s, const char **parsed_until,
                                     int did, int uid, struct ExpandoParseError *error)
{
  return expando_parse_enclosed_expando(s, parsed_until, did, uid, '@', error);
}
...
/**
 * IndexFormatData - Expando definitions
 *
 * Config:
 * - $attribution_intro
 * - $attribution_trailer
...
const struct ExpandoDefinition IndexFormatData[] = {
  ...
  { "A",  "reply-to",            ED_ENVELOPE, ED_ENV_REPLY_TO,            E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "B",  "list-address",        ED_ENVELOPE, ED_ENV_LIST_ADDRESS,        E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "b",  "mailbox-name",        ED_MAILBOX,  ED_MBX_MAILBOX_NAME,        E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "C",  "number",              ED_EMAIL,    ED_EMA_NUMBER,              E_TYPE_NUMBER, E_FLAGS_NO_FLAGS, NULL },
  { "c",  "size",                ED_EMAIL,    ED_EMA_SIZE,                E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "d",  "date-format",         ED_EMAIL,    ED_EMA_DATE_FORMAT,         E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "D",  "date-format-local",   ED_EMAIL,    ED_EMA_DATE_FORMAT_LOCAL,   E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "E",  "thread-count",        ED_EMAIL,    ED_EMA_THREAD_COUNT,        E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "e",  "thread-number",       ED_EMAIL,    ED_EMA_THREAD_NUMBER,       E_TYPE_NUMBER, E_FLAGS_NO_FLAGS, NULL },
  { "f",  "from-full",           ED_ENVELOPE, ED_ENV_FROM_FULL,           E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "F",  "sender",              ED_ENVELOPE, ED_ENV_SENDER,              E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "g",  "tags",                ED_EMAIL,    ED_EMA_TAGS,                E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "G",  "tags-transformed",    ED_EMAIL,    ED_EMA_TAGS_TRANSFORMED,    E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "H",  "spam",                ED_ENVELOPE, ED_ENV_SPAM,                E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "I",  "initials",            ED_ENVELOPE, ED_ENV_INITIALS,            E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "i",  "message-id",          ED_ENVELOPE, ED_ENV_MESSAGE_ID,          E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "J",  "thread-tags",         ED_EMAIL,    ED_EMA_THREAD_TAGS,         E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "K",  "list-empty",          ED_ENVELOPE, ED_ENV_LIST_EMPTY,          E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "L",  "from-list",           ED_EMAIL,    ED_EMA_FROM_LIST,           E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "l",  "lines",               ED_EMAIL,    ED_EMA_LINES,               E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "m",  "message-count",       ED_MAILBOX,  ED_MBX_MESSAGE_COUNT,       E_TYPE_NUMBER, E_FLAGS_NO_FLAGS, NULL },
  { "M",  "thread-hidden-count", ED_EMAIL,    ED_EMA_THREAD_HIDDEN_COUNT, E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "n",  "name",                ED_ENVELOPE, ED_ENV_NAME,                E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "N",  "score",               ED_EMAIL,    ED_EMA_SCORE,               E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "O",  "save-folder",         ED_EMAIL,    ED_EMA_SAVE_FOLDER,         E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "P",  "percentage",          ED_MAILBOX,  ED_MBX_PERCENTAGE,          E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "q",  "newsgroup",           ED_ENVELOPE, ED_ENV_NEWSGROUP,           E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "R",  "cc-all",              ED_ENVELOPE, ED_ENV_CC_ALL,              E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "r",  "to-all",              ED_ENVELOPE, ED_ENV_TO_ALL,              E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "S",  "status-flags",        ED_EMAIL,    ED_EMA_STATUS_FLAGS,        E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "s",  "subject",             ED_ENVELOPE, ED_ENV_SUBJECT,             E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "t",  "to",                  ED_ENVELOPE, ED_ENV_TO,                  E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "T",  "to-chars",            ED_EMAIL,    ED_EMA_TO_CHARS,            E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "u",  "username",            ED_ENVELOPE, ED_ENV_USERNAME,            E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "v",  "first-name",          ED_ENVELOPE, ED_ENV_FIRST_NAME,          E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "W",  "organization",        ED_ENVELOPE, ED_ENV_ORGANIZATION,        E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "X",  "attachment-count",    ED_EMAIL,    ED_EMA_ATTACHMENT_COUNT,    E_TYPE_NUMBER, E_FLAGS_NO_FLAGS, NULL },
  { "x",  "x-comment-to",        ED_ENVELOPE, ED_ENV_X_COMMENT_TO,        E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "Y",  "thread-x-label",      ED_ENVELOPE, ED_ENV_THREAD_X_LABEL,      E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "y",  "x-label",             ED_ENVELOPE, ED_ENV_X_LABEL,             E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "Z",  "combined-flags",      ED_EMAIL,    ED_EMA_COMBINED_FLAGS,      E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "zc", "crypto-flags",        ED_EMAIL,    ED_EMA_CRYPTO_FLAGS,        E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "zs", "status-flags",        ED_EMAIL,    ED_EMA_STATUS_FLAGS,        E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
...
static const struct ExpandoDefinition StatusFormatData[] = {
  ...
  { "F", "flagged-count",   ED_INDEX,  ED_IND_FLAGGED_COUNT,   E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "f", "mailbox-path",    ED_INDEX,  ED_IND_MAILBOX_PATH,    E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "h", "hostname",        ED_GLOBAL, ED_GLO_HOSTNAME,        E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "L", "limit-size",      ED_INDEX,  ED_IND_LIMIT_SIZE,      E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "l", "mailbox-size",    ED_INDEX,  ED_IND_MAILBOX_SIZE,    E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "M", "limit-count",     ED_INDEX,  ED_IND_LIMIT_COUNT,     E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "m", "message-count",   ED_INDEX,  ED_IND_MESSAGE_COUNT,   E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "n", "new-count",       ED_INDEX,  ED_IND_NEW_COUNT,       E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "o", "old-count",       ED_INDEX,  ED_IND_OLD_COUNT,       E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "P", "percentage",      ED_MENU,   ED_MEN_PERCENTAGE,      E_TYPE_NUMBER, E_FLAGS_NO_FLAGS, NULL },
  { "p", "postponed-count", ED_INDEX,  ED_IND_POSTPONED_COUNT, E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "R", "read-count",      ED_INDEX,  ED_IND_READ_COUNT,      E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "r", "readonly",        ED_INDEX,  ED_IND_READONLY,        E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "s", "sort",            ED_GLOBAL, ED_GLO_SORT,            E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "S", "sort-aux",        ED_GLOBAL, ED_GLO_SORT_AUX,        E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "t", "tagged-count",    ED_INDEX,  ED_IND_TAGGED_COUNT,    E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "T", "use-threads",     ED_GLOBAL, ED_GLO_USE_THREADS,     E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { "u", "unread-count",    ED_INDEX,  ED_IND_UNREAD_COUNT,    E_TYPE_NUMBER, E_FLAGS_OPTIONAL, NULL },
  { "V", "limit-pattern",   ED_INDEX,  ED_IND_LIMIT_PATTERN,   E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { "v", "version",         ED_GLOBAL, ED_GLO_VERSION,         E_TYPE_STRING, E_FLAGS_NO_FLAGS, NULL },
  { NULL, NULL, 0, -1, -1, 0, NULL }
  // clang-format on
};
...
static struct ConfigDef MainVars[] = {
  ...
  { "status_format", DT_EXPANDO, IP "-%r-NeoMutt: %D [Msgs:%<M?%M/>%m%<n? New:%n>%<o? Old:%o>%<d? Del:%d>%<F? Flag:%F>%<t? Tag:%t>%<p? Post:%p>%<b? Inc:%b>%<l? %l>]---(%<T?%T/>%s/%S)-%>-(%P)---", IP &StatusFormatData, NULL,
...
/**
 * MixFormatData - Expando definitions
 *
 * Config:
 * - $mix_entry_format
 */
static const struct ExpandoDefinition MixFormatData[] = {
  ...
  { "s", "short-name",   ED_MIXMASTER, ED_MIX_SHORT_NAME,   E_TYPE_STRING, E_FLAGS_OPTIONAL, NULL },
  { NULL, NULL, 0, -1, -1, 0, NULL }
  // clang-format on
};
